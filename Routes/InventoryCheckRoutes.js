...
inventoryCheckRoutes.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const lastIssue = await inventoryCheck
    .find()
    .sort({ _id: -1 })
    .limit(1)
     let code = ''
     ...
     if (lastIssue?.length > 0) {
       const lastCode = lastIssue[0]?.checkCode
       const lastIssueDate = lastCode?.split(`${process.env.PREFIX_CODE_CBB}`)[1]?.split('-')[0] // Lấy ngày từ mã code trước đó
       ...
       if (lastIssueDate === today) {
         ...
         code = `${process.env.PREFIX_CODE_CBB}${today}-${(lastCodeNumber + 1).toString().padStart(4, '0')}`
       } else {
         // Nếu không phải cùng ngày, reset số thứ tự về 0001
         code = `${process.env.PREFIX_CODE_CBB}${today}-0001`
       }
     } else {
