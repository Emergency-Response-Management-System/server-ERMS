...
inventoryRoutes.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (oh === "OH2") {
      countFilter = { $gt: config?.quantityWarning ? +config?.quantityWarning :  30 };
    } else if (oh === "OH1") {
      countFilter = { $gte: 1, $lte: config?.quantityWarning ? +config?.quantityWarning :  30 };
    } else if (oh === "OH0") {
      countFilter = { $lte: 0 };
    } else {
                ...
                if (
                  Math.round(
                    (moment(item?.products[i]?.expDrug) - moment(Date.now())) /
                      (config?.quantityDate ? +config?.quantityDate * 24 * 60 * 60 * 1000 :  30 * 24 * 60 * 60 * 1000),
