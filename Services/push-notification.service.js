...
async function sendNotificationsExpDrug() {
  ...
  const config = await Configuration.findOne({ _id: 'default' }).lean()

  const now = new Date();
  const thirtyDaysFromNow = new Date(now.getTime() + config?.quantityDate ? +config?.quantityDate * 24 * 60 * 60 * 1000 :  30 * 24 * 60 * 60 * 1000);
...
async function sendNotificationsInventory() {
  const products = await Inventory.find({ count: { $lt: config?.quantityWarning ? +config?.quantityWarning  :  30  } }).populate(
    "idDrug",
    "name",
  );
  if (products.length > 0) {
...
async function sendNotificationsInventoryGROUP() {
  const pipeline = [
    {
      $match: { count: { $lt: config?.quantityWarning ? +config?.quantityWarning  :  30 } },
    },
    {
      $lookup: {
        from: "products",
